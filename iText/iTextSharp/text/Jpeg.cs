...
/*
 * $Id: Jpeg.cs,v 1.4 2003/08/21 22:31:51 geraldhenson Exp $
 * $Name:  $
 *
...
namespace iTextSharp.text {
	...
	public class Jpeg : Image, IElement {
		...
		public static int NOT_A_MARKER = -1;
    
		/// <summary> This is a type of marker. </summary>
		public static int VALID_MARKER = 0;
    
		/// <summary> Acceptable Jpeg markers. </summary>
		public static int[] VALID_MARKERS = {0xC0, 0xC1, 0xC2};
    
		/// <summary> This is a type of marker. </summary>
		public static int UNSUPPORTED_MARKER = 1;
		...
		/// <summary> This is a type of marker. </summary>
		public static int NOPARAM_MARKER = 2;
		...
		public static int M_APP0 = 0xE0;
		public static int M_APPE = 0xEE;
		...
		public Jpeg(Uri Uri, float width, float height) : this(Uri) {
			scaledWidth = width;
			scaledHeight = height;
		}
		...
		public Jpeg(Stream str) : base(str) {
			processParameters();
		}
				...
				while (true) {
					if (istr.ReadByte() == 0xFF) {
						...
						if (firstPass && marker == M_APP0) {
							...
							skip(istr, len - 2 - bcomp.Length - 7);
							continue;
						}
						if (marker == M_APPE) {
							len = getShort(istr);
							...
							if (byteappe.Length > 12) {
								...
								if (appe.ToLower() == "adobe") {
									// Look for a valid marker (0xC2), so that
									// we're at that right position for next step
									while (true) {
										if (istr.ReadByte() == 0xFF) 
											marker = istr.ReadByte();
										if (markerType(marker) == VALID_MARKER) {
											firstPass = false;
											break;
										}
									}
						...
						if (markertype == VALID_MARKER) {
							...
							if (invert) {
								//Experimental for MAC support I guess
								//scaledHeight = getShortInverted(istr);
								//scaledWidth = getShortInverted(istr);
