...
/*
 * $Id: DocWriter.cs,v 1.2 2003/03/12 20:10:08 geraldhenson Exp $
 * $Name:  $
 *
...
namespace iTextSharp.text {
	...
	public abstract class DocWriter : IDocListener {
		...
		/// <summary> This is some byte that is often used. </summary>
		public static byte EQUALS = (byte)'=';

		/// <summary> This is some byte that is often used. </summary>
		public static byte QUOTE = (byte)'\"';

		/// <summary> This is some byte that is often used. </summary>
		public static byte GT = (byte)'>';

		/// <summary> This is some byte that is often used. </summary>
		public static byte FORWARD = (byte)'/';
		 ...
		 * a Watermark.
		 *
		 * @return  false (because watermarks aren't supported by default).
		 */
		/// <summary>
		/// Sets the Watermark.
		/// </summary>
		...
		/// <returns>false (because watermarks aren't supported by default).</returns>
		...
		/// <returns></returns>
		protected bool writeMarkupAttributes(IMarkupAttributes mAtt) {
			IEnumerator attributeIterator = mAtt.MarkupAttributeNames.GetEnumerator();
			foreach(string name in mAtt.MarkupAttributeNames) {
				write(name, mAtt.getMarkupAttribute(name));
			}
			return (mAtt.MarkupAttributeNames.Count != 0);
		}
		...
		/// <returns>a boolean</returns>
		protected static bool hasMarkupAttributes(IElement element) {
			return (element is IMarkupAttributes &&
				!(((IMarkupAttributes)element).MarkupAttributeNames.Count == 0));
		}
