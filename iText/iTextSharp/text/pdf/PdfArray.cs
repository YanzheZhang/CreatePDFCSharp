...
/*
 * $Id: PdfArray.cs,v 1.1.1.1 2003/02/04 02:57:02 geraldhenson Exp $
 * $Name:  $
 *
 ...
 */
    ...
    public override byte[] toPdf(PdfWriter writer) {
        try {
            ...
            str.WriteByte(System.Convert.ToByte('['));
            
			bool first = true;
			foreach(PdfObject obj in arrayList) {
				if (first) {
					byte[] tmp = obj.toPdf(writer);
